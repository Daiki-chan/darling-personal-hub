import { NUMBERS_DATA } from "@/lib/portfolio-data";

export function PortfolioNumbers() {
  return (
    <section className="phuc-numbers section-shell section-space" aria-label="Key Performance Indicators">
      <div className="phuc-numbers__editorial-grid">
        {NUMBERS_DATA.map((item) => (
          <div key={item.label} className="phuc-editorial-stat">
            <span className="phuc-stat-val">{item.number}</span>
            <span className="phuc-stat-lbl">{item.label}</span>
          </div>
        ))}
      </div>
    </section>
  );
}
